...
f_test_connection () {
      ...
      ERROR=$?
      if [ $ERROR -ne 0 ] ; then
         f_message $1 "NOK" "Ping Test Network Connection" " \n\Z1\Zb  No network connection, cannot get list of upgradable packages.\Zn"
      else
...
f_about () {
      ...
      # so it is not printed. Also do not print lines beginning with "#@@#".
      sed -n -e 's/^#@@/##/' -e 's/^#@//'p $THIS_DIR/$THIS_FILE >> $TEMP_FILE
      #
           ...
           "dialog" | "whiptail")
              ...
              if [ -r "$4" ] ; then
                 ...
                 if [ "$2" = "OK" ] ; then
                    ...
                    f_msg_ui_str_nok $1 $2 "$3" "$4" $YBOX $XBOX
                 fi
              fi
              ;;
           *)
...
f_edit_script_files () {
      ...
      do
          ...
          if [ -w ~/$1/$SCRIPT ] ; then
            ...
            f_message $1 "Error" "\Z1\Zb!!! Cannot modify $1/$SCRIPT !!!\Zn"
          fi
      done
      #
